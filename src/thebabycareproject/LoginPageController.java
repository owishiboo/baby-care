...
public class LoginPageController implements Initializable {
    ...
    void signin(ActionEvent event) throws IOException, SQLException {
        ...
        else if(password.getText().isEmpty())labelPass.setText("Please enter your password");
        else if(valid.validateEmail(email) && valid.validatePassword(password) && checkDB_with_pass(email,password)==true){
            email_txt=email.getText();
            gl.notification("", "Welcome to baby care!");
            gl.loadUI(event, "homePage");
            gl.setMusic();
        }
        else if(!valid.validatePassword(password) && !valid.validateEmail(email)) {
             noAccount.setText("Please fill these fields correctly");
        }
        else noAccount.setText("We don't have this account");       
        }
     ...
     * @throws SQLException if the user database can't be connected or found
     */
    boolean checkDB_with_pass(TextField email, PasswordField password){
            Connection connection ;
        try {
            connection = DriverManager.getConnection("jdbc:mysql://localhost/java2db", "root", ""); //address userid password
            ...
            System.out.println("Exception - Not connected");
            return false;
